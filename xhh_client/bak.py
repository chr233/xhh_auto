...
class HeyboxClient():
    ...
    def tools_follow_followers(self, fastmode: bool = True) -> bool:
        ...
        try:
            if fastmode:
                ...
                fliteredlist = self.filte_userlist(
                    fanlist, {'RelationType': RelationType.HeFollowedMe})
                if not fliteredlist:
            ...
            fliteredlist = self.filte_userlist(
                fanlist, {'RelationType': RelationType.HeFollowedMe})
            if fliteredlist:
                self.batch_userlist_operate(
                    fliteredlist, OperateType.FollowUser)
    ...
    def tools_unfollow_singlefollowers(self, value: int = 100) -> bool:
        ...
        try:
            ...
            fliteredlist = self.filte_userlist(
                followlist, {'RelationType': RelationType.IFollowedHim})
            fliteredlist = self.filte_userlist(
                fliteredlist, {'FollowValue': value})
            if fliteredlist:
                self.batch_userlist_operate(
                    fliteredlist, OperateType.UnFollowUser)
            return(True)
        except ClientException as e:
    ...
    def tools_follow_recommand(self, count: int = 10, value: int = 100) -> bool:
        ...
        try:
            ...
            fliteredlist = self.filte_userlist(
                recfollowlist, {'FollowValue': value})
            if fliteredlist:
                self.batch_userlist_operate(
                    fliteredlist, OperateType.FollowUser)
            return(True)
        ...
        if idsetlist:
            ...
            for idobj in idsetlist:
                try:
                    linkid, newsid = idobj
                    if view:
                        has_viedo, is_liked = self.get_news_links(
                            linkid, newsid, index)
                        if not has_viedo:
                            self.get_news_body(newsid, index, fastmode)
                        else:
                            self.get_video_title(
                                linkid, newsid, index, fastmode)
    ...
    def batch_userlist_operate(self, useridlist: list, operatetype: int = 1):
        ...
        def follow_user(userid: int):
            ...
            resp = self.Session.post(
                url=url, data=data, params=self._params, headers=headers, cookies=self._cookies)
            jsondict = resp.json()
    ...
    def get_news_links_ex(self, linkid: int, newsid: int, index: int = 1):
        ...
        try:
            ...
            self.logger.debug(
                f'标题[{title}] 作者[{author_name}] @{author_id} [{author_level}级]')
            self.logger.debug(
                f'点击[{click}] 点赞[{like_count}] 评论[{comment_count}]')
        ...
        def _get_user_post_list(offset: int = 0):
            ...
            for postlink in jsondict['post_links']:
                try:
                    ...
                    if is_liked == False or ignoreliked == False:
                        postlist.append(
                            (linkid, FollowPostType.PostLink, is_liked))
                except KeyError as e:
    ...
    def get_ads_info(self):
        ...
        self.__flush_params()
        resp = self.Session.get(
            url=url, params=self._params, headers=self._headers, cookies=self._cookies)
    ...
    def filte_userlist(self, userlist: list, filtersetting: dict):
        ...
        def filter_by_follow(userobj: list) -> bool:
            ...
            try:
                ...
                follow_num, fan_num, awd_num = self.get_user_profile(
                    userobj[0])
    ...
    def share_news(self, newsid: int, index: int = 1):
        ...
        def check_share_task_qq():
            ...
            resp = self.Session.get(
                url=url, headers=self._headers, params=params, cookies=self._cookies)
            try:
                jsondict = resp.json()
                self.__check_status(jsondict)
    ...
    def send_message(self, userid: int, message: str):
        ...
        resp = self.Session.post(
            url=url, headers=headers, cookies=self._cookies, params=params, data=data)
        try:
            jsondict = resp.json()
            self.__check_status(jsondict)
    ...
    def get_game_detail(self, appid: int):
        ...
        resp = self.Session.get(
            url=url, headers=self._headers, cookies=self._cookies, params=params)
        try:
            jsondict = resp.json()
            self.__check_status(jsondict)
            ...
            if is_free:
                ...
                self.logger.debug(
                    f'价格[{initial_price}￥ - {discount}% = {current_price}￥{" 史低" if current_price==lowest_price else ""}]')
            else:
                self.logger.debug('价格[不支持主机游戏]')
            self.logger.debug(
                f'评分[{score if score != -1 else "评分较少"}|{game_review_summary}] 发布日期[{release_date}]')
    ...
    def get_game_detail_ex(self, appid: int):
        ...
        try:
            ...
            self.logger.debug(f'名称[{name} {name_en}] @ {appid}')
            self.logger.debug(
                f'开发商[{developer}] 发行商[{publishers}] 发布日期[{release_date}]')
    ...
    def do_communitu_surver(self):
        ...
        def get_community_survey():
            ...
            self.__flush_params()
            resp = self.Session.get(
                url=url, headers=headers, params=self._params, cookies=cookies)
            html = resp.text
        ...
        def get_bbs_qa_state():
            ...
            resp = self.Session.get(
                url=url, headers=self._headers, cookies=cookies)
            try:
                jsondict = resp.json()
                self.__check_status(jsondict)
    ...
    def get_daily_task_detail(self):
        ...
        try:
            ...
            self.logger.debug(
                f"签到{is_sign}|分享{is_share_news}|{is_share_comment}|点赞{is_like}")
    ...
    def get_ex_task_detail(self):
        ...
        try:
            jsondict = resp.json()
            self.__check_status(jsondict)
            band = public = bandwx = exam = push = profile = evaluate = BoolenString(
                True)
            ...
            self.logger.debug(
                f'绑定{band}|微信{bandwx}|公开{public}|评价{evaluate}|答题{exam}|推送{push}|资料{profile}')
    ...
    def get_my_data(self):
        ...
        try:
            ...
            sign_in_streak = int(
                jsondict['result']['task_list'][0]['tasks'][0]['sign_in_streak'])
